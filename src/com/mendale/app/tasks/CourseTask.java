...
public class CourseTask {
	 ...
	 * @param context
	 */
	public CourseTask(Context context, Handler mhandler) {
			...
			if (conn.getResponseCode() == 200) {
				...
				if (!Utils.isEmpty(responseData)) {
					...
					if (json.get("list") != null) {
						JSONArray course = json.getJSONArray("list");
						List<CoursePoJo> courseList = new ArrayList<CoursePoJo>();
						for (int i = 0; i < course.length(); i++) {
							CoursePoJo item = gson.fromJson(course.get(i)
									.toString(), new TypeToken<CoursePoJo>() {
							}.getType());
							//
							courseList.add(item);
						}
