...
public class CourseListTask {
	 ...
	 * @param context
	 */
	public CourseListTask(Context context, Handler mhandler) {
			...
			if (conn.getResponseCode() == 200) {
				...
				if (!Utils.isEmpty(responseData)) {
					...
					if (json.get("list") != null) {
						...
						List<CourseListPojo> recordList = new ArrayList<CourseListPojo>();
						for (int i = 0; i < record.length(); i++) {
							CourseListPojo item = gson.fromJson(record.get(i)
									.toString(), new TypeToken<CourseListPojo>() {
							}.getType());
							//
							recordList.add(item);
						}
