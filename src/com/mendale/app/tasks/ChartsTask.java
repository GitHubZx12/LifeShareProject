...
public class ChartsTask {
	 ...
	 * @param context
	 */
	public ChartsTask(Context context, Handler mhandler) {
	...
	public void send(int id, String encoding, String action) {
		...
		try {
			...
			if (conn.getResponseCode() == 200) {
				...
				Log.e("http", responseData.toString());
				// responseData是请求服务器返回的结果（json格式的）
				if (!Utils.isEmpty(responseData)) {
					JSONObject json = new JSONObject(responseData);
					if (json.get("list") != null) {
						JSONArray charts = json.getJSONArray("list");
						// List<ChartsPoJo> chartsList = new
						// ArrayList<ChartsPoJo>();
						// for (int i = 0; i < charts.length(); i++) {
						// ChartsPoJo item =
						// gson.fromJson(charts.get(i).toString(), new
						// TypeToken<ChartsPoJo>() {
						// }.getType());
						// //
						// Log.e("tag",chartsList.toString());
						// chartsList.add(item);
						// }
						List<ChartsPoJo> chartsList = gson.fromJson(responseData,new TypeToken<List<ChartsPoJo>>() {
								}.getType());
						Log.e("tag",chartsList.toString());
						msg.what = 1;
						msg.obj = chartsList;
