...
public class CommentListTask {
	 ...
	 * @param context
	 */
	public CommentListTask(Context context, Handler mhandler,List<CommentList>commentList) {
		...
		try {
			...
			if (conn.getResponseCode() == 200) {
				...
				if (!Utils.isEmpty(responseData)) {
					...
					JSONArray commentData=json.getJSONArray("comment_list");
//					List<CommentList>commentData=new ArrayList<CommentList>();
					for(int i=0;i<commentData.length();i++){
						CommentList item = gson.fromJson(commentData.get(i).toString(), new TypeToken<CommentList>() {
						}.getType());
						item.save(context);
						commentList.add(item);
					}
					Log.e("tag", commentList.size()+"-");
