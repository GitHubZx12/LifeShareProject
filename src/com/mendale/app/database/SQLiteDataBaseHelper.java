...
public class SQLiteDataBaseHelper implements DataBaseConstDefine {
	...
	public SQLiteDataBaseHelper(Context context) {
		// 方案1 开发测试阶段使用
		//次路径下面有咩有该目录
		File file = new File(SdCardPathUtil.getSDPath() + "/" + URLS.DATABASE_NAME);
		if (file.exists()) {
		...
		// 方案2 生产阶段使用
		if (db == null) {
			db = context.openOrCreateDatabase(URLS.DATABASE_NAME, Context.MODE_PRIVATE, null);
			if (URLS.DB_VERSION == db.getVersion()) {
				// 版本相同,不需要更新db文件
			}
			else {
				// 程序员手动设置本次发布版本的数据库
				...
				db.execSQL(PROFILEINFO_CREATE_SQL);// 配置记录表
				db.execSQL(MESSAGEINFO_CREATE_SQL);// 消息记录表
				db.setVersion(URLS.DB_VERSION);// 设置成本次版本
			}
		}
	}
